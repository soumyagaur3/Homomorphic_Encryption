...
def polymul(x, y, modulus, poly_mod):
    ...
    if n == 1:
        ...
        X = (x[0] + x[1]) * (y[0] + y[1]) % modulus
        Y = x[0] * y[0] % modulus
        Z = x[1] * y[1] % modulus
        mid = (X - Y - Z) % modulus
        return np.int64(np.round(poly.polydiv(np.array([Y] + [mid] + [Z]), poly_mod)[1] % modulus))
...
def encrypt(pk, size, q, t, poly_mod, pt):
    """Encrypt an integer or a string.
    Args:
        pk: public-key.
        ...
        t: plaintext modulus.
        poly_mod: polynomial modulus.
        pt: integer or string to be encrypted.
    Returns:
        Tuple representing a ciphertext.
    """
    delta = q // t
    if isinstance(pt, str):
        pt = [ord(char) for char in pt]  # Convert string to ASCII values
    scaled_m = [delta * (char % t) % q for char in pt]
...
def decrypt(sk, size, q, t, poly_mod, ct):
    ...
    Args:
        ...
        ct: ciphertext.
    Returns:
        ASCII values representing the plaintext.
    """
    scaled_pt = polyadd(
...
# Encryption
pt1 = input("Enter string or number for pt1: ")
pt2 = input("Enter string or number for pt2: ")
cst1 = input("Enter string or number for cst1: ")
cst2 = input("Enter string or number for cst2: ")
...
print(f"Time of the process: {end_time-start_time} seconds")
